...
import { canAccessRep, canAccessCustomerAsAdmin } from '../services/adminScope';
 ...
 * يُوضع **قبل** حارس الأدمن كي يبلغه المندوب.
 */
router.get('/mine', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const tid = tenantId(req);
    const salesRepId = req.user!.role === 'SALES_REP' ? req.user!.id : String(req.query.salesRepId || '');
    ...
    const visits = await prisma.repVisit.findMany({
      where: {
        ...
        customerId: { in: route.stops.map(s => s.customerId) },
    ...
    res.json({
      ...
      data: {
        ...
        doneCount: route.stops.filter(s => doneAt.has(s.customerId)).length,
  ...
  try {
    ...
    res.json({
      ...
      data: routes.map(r => ({
        ...
        doneToday: r.stops.filter(s => seen.has(key(r.salesRepId, s.customerId))).length,
      })),
    });
  } catch (err) { next(err); }
});
...
router.get('/:id', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!r) { res.status(404).json({ success: false, message: 'خط السير غير موجود' }); return; }
    res.json({
      success: true,
      data: {
