...
const receiptSchema = z.object({
  ...
  receiptDate: z.string().optional(), // يسمح للأدمن بإصدار سند بتاريخ قديم
...
router.post('/', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const docDate = body.receiptDate ? new Date(body.receiptDate) : undefined; // تاريخ مخصّص (قديم) إن وُجد
    ...
    const receipt = await prisma.$transaction(async tx => {
      ...
      if (body.invoiceAllocations?.length) {
        await tx.receiptInvoice.createMany({
          data: body.invoiceAllocations.map(a => ({
            receiptId: rcp.id,
        ...
        for (const alloc of body.invoiceAllocations) {
          await tx.invoice.update({
            ...
            data: {
              paidAmt: { increment: alloc.amount },
              remainingAmt: { decrement: alloc.amount },
            },
          });
        }
      }

      await postReceiptEntries(tx as never, rcp.id, body.customerId, body.amount, docDate);
      return rcp;
    });
...
router.patch('/:id/cancel', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const updated = await prisma.$transaction(async tx => {
      const rcp = await tx.receipt.update({
        where: { id: req.params.id },
      ...
      for (const item of receipt.invoiceItems) {
        await tx.invoice.update({
          ...
          data: {
            paidAmt: { decrement: Number(item.amount) },
            remainingAmt: { increment: Number(item.amount) },
          },
        });
      }

      await reverseReceiptEntries(tx as never, rcp.id, rcp.customerId, Number(rcp.amount));
      return rcp;
    });
