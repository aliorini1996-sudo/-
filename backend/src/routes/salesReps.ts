...
router.get('/', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const where = search
      ? { OR: [{ name: { contains: search } }, { phone: { contains: search } }, { username: { contains: search } }] }
      : {};
  ...
  try {
    const rep = await prisma.salesRep.findUnique({
      ...
      select: {
        ...
        canChangePrice: true, maxDiscountPct: true, canSellBelowPrice: true,
        canCreateReceipt: true, canEditReceipt: true, canCancelReceipt: true,
        canAddCustomer: true, canEditCustomer: true, canViewStatement: true, createdAt: true,
      },
    });
...
router.post('/', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    // فحص التكرار برسائل واضحة لكل حقل
    const dup = await checkRepDuplicates(rest.username, rest.phone, rest.email);
    if (dup) { res.status(409).json({ success: false, message: dup }); return; }

    const passwordHash = await bcrypt.hash(password, 10);
    const rep = await prisma.salesRep.create({
      data: { ...rest, passwordHash, email: rest.email || null },
...
// يتحقق من تفرّد اسم المستخدم والجوال والبريد، ويُعيد رسالة عربية إن وُجد تكرار
async function checkRepDuplicates(username: string, phone: string, email?: string, excludeId?: string): Promise<string | null> {
  ...
  if (await prisma.salesRep.findFirst({ where: { phone, ...notSelf } })) return 'رقم الجوال مستخدم مسبقاً';
  if (email && await prisma.salesRep.findFirst({ where: { email, ...notSelf } })) return 'البريد الإلكتروني مستخدم مسبقاً';
  return null;
}
...
router.put('/:id', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    // منع تعارض اسم المستخدم/الجوال/البريد مع مندوب آخر
    if (rest.username || rest.phone || rest.email) {
      const current = await prisma.salesRep.findUnique({ where: { id: req.params.id } });
  ...
  try {
    ...
    const [invoices, receipts] = await Promise.all([
      prisma.invoice.aggregate({
        where: { salesRepId: req.params.id, status: 'CONFIRMED', ...(dateFilter && { invoiceDate: dateFilter }) },
        _count: { id: true },
        _sum: { total: true },
      }),
      prisma.receipt.aggregate({
        where: { salesRepId: req.params.id, status: 'ACTIVE', ...(dateFilter && { receiptDate: dateFilter }) },
        _count: { id: true },
        _sum: { amount: true },
      }),
    ]);
