...
router.post('/renew', renewLimiter, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // التجديد يوقّع توكناً جديداً بلا علم impersonated فتسقط كل حراسة الانتحال (تغيير كلمة المرور، عدّ الاستهلاك، سطر تدقيق
    // ربط ZATCA ووسم actorId) وتُنسب الكتابة لمدير الشركة بلا أثر، ويُجدَّد بلا نهاية. المالك يدخل مجدداً من لوحة المنصّة.
    ...
    if (req.user.role === 'SALES_REP') {
      const rep = await prisma.salesRep.findUnique({
        ...
        select: { id: true, name: true, phone: true, email: true, username: true, tenantId: true,
          ...
          canSellOnCredit: true, canSellOnInstallment: true, canSellInCash: true,
