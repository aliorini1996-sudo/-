/**
 * محرّك شكل خطّ السير — `buildRouteShape` وأدواته الهندسية.
 *
 * الخلل الذي يحرسه: كنّا نمرّر نقاط اليوم كلّها إلى مطابقة المسارات، وهي ترفض
 * الطلب **برمّته** إذا تباعد زوج نقاط أكثر من ٢ كم (400 breakage distance) —
 * وهو حال أغلب الأيام لأن المتصفّح لا يلتقط موقعاً والتطبيق مغلق. فيسقط النظام
 * على النقاط الخام ويرسم خطوطاً مستقيمة فوق البحر والمباني (بلاغ المالك بالصورة).
 *
 * الاعتمادات تُحقن، فالمنطق كلّه يُختبر بلا شبكة ولا مفاتيح.
 */
...
/** نقطة على بعد `m` متراً شمالاً تقريباً */
...
/** أثر كثيف: `n` نقطة متتالية بفارق ٣٠ متراً (قيادة داخل حيّ) */
const denseRun = (start: LatLng, n: number, t0 = 0): ShapePoint[] =>
  Array.from({ length: n }, (_, i) => at(north(start, i * 45), t0 + i * 8));

// اعتمادات مزيّفة: تُعيد خطوطاً يمكن تمييزها، وتُحصي النداءات
...
test('هافرساين يقيس مسافة معلومة بدقّة معقولة', () => {
  // القطيف ← الخبر ≈ ٣٠ كم بخطّ مستقيم
  const km = haversineM(QATIF, KHOBAR) / 1000;
  assert.ok(km > 25 && km < 35, `المسافة ${km.toFixed(1)} كم خارج المتوقّع`);
  assert.equal(Math.round(haversineM(DAMMAM, DAMMAM)), 0);
});

test('القسمة تفصل عند تجاوز حدّ التباعد وحده', () => {
  const pts = [QATIF, DAMMAM, KHOBAR].map((p, i) => at(p, i * 600));
  assert.equal(splitRuns(pts).length, 3, 'ثلاث نقاط متباعدة ⇒ ثلاث جَرَيات');

  const close = [DAMMAM, north(DAMMAM, 300), north(DAMMAM, 600)].map((p, i) => at(p, i * 8));
  assert.equal(splitRuns(close).length, 1, 'نقاط متقاربة ⇒ جَرْية واحدة');
});

test('حدّ القسمة دون سقف المزوّد الصلب (٢٠٠٠م)', () => {
  assert.ok(BREAK_M < 2000, 'تجاوزُ الحدّ يُفشل الطلب كاملاً لا المقطع وحده');
...
test('التحضير يرتّب زمنياً ويُسقط الدقّة الرديئة ويدمج الوقفة', () => {
  const pts: ShapePoint[] = [
    ...
    { ...north(DAMMAM, 5000), capturedAt: new Date(2500), accuracy: 900 }, // دقّة رديئة
  ];
  const out = prepare(pts);
  assert.equal(out.length, 2, 'تبقى نقطتان: البداية والحركة الحقيقية');
  assert.equal(out[0].capturedAt, pts[1].capturedAt, 'الأقدم أولاً');
});

test('كل النقاط رديئة الدقّة ⇒ نستعملها بدل إفراغ المسار', () => {
  const pts: ShapePoint[] = [
...
test('السيناريو المُبلَّغ عنه: ثلاث وقفات متباعدة ⇒ مسار مُرجَّح لا خطّ مستقيم', async () => {
  const { deps, calls } = fakeDeps();
  const pts = [QATIF, DAMMAM, KHOBAR].map((p, i) => at(p, i * 900));
  const shape = await buildRouteShape(pts, deps);

  assert.equal(calls.match, 0, 'لا مطابقة على نقاط متباعدة — كانت تُفشل الطلب');
  assert.equal(calls.route, 1, 'نداء توجيه واحد يغطّي اليوم كلّه');
  assert.equal(shape.segments.length, 1);
  assert.equal(shape.segments[0].kind, 'inferred', 'مُرجَّح — لم نرصد الطريق');
  assert.ok(shape.segments[0].points.some(p => p.lat === 0.222), 'الخطّ من محرّك التوجيه لا خاماً');
...
test('يوم مختلط: أثر كثيف ثم انتقال بعيد ثم أثر كثيف', async () => {
  ...
  assert.equal(calls.match, 2, 'مطابقة لكل جَرْية كثيفة');
  assert.equal(calls.route, 1, 'وتوجيهٌ للفراغ بينهما');
  assert.deepEqual(shape.segments.map(s => s.kind), ['observed', 'inferred', 'observed'],
    'الترتيب الزمني محفوظ: مرصود ← مُرجَّح ← مرصود');
  assert.ok(shape.observedMeters > 0 && shape.inferredMeters > 0);
});

test('فشل التوجيه ⇒ خطّ مستقيم موسوم **مُرجَّحاً** لا مرصوداً', async () => {
  ...
  assert.equal(shape.segments[0].kind, 'inferred',
    'التخمين يبقى تخميناً حتى حين يفشل المحرّك — وسمُه مرصوداً كذبٌ على المشرف');
});

test('فشل المطابقة ⇒ **raw** لا «مرصود» — ولا يدخل عدّاد المرصود', async () => {
  ...
  assert.equal(shape.segments[0].kind, 'raw',
    'وسمُ الخطّ المستقيم «مرصوداً» يصبغ التخمين بلون الدليل — وهو عين العلّة');
  assert.equal(shape.observedMeters, 0);
  assert.ok(shape.rawMeters > 0);
  assert.ok(!shape.segments[0].points.some(p => p.lat === 0.111));
});

test('تعذّر المزوّد كلّياً (لا مفتاح/حصّة منتهية) ⇒ degraded=true', async () => {
  const { deps } = fakeDeps({ matchFails: true, routeFails: true });
  const pts = [...denseRun(DAMMAM, 20, 0), ...denseRun(KHOBAR, 20, 3000)];
  const shape = await buildRouteShape(pts, deps);
  assert.equal(shape.degraded, true, 'ما يُعرض خطوطٌ مستقيمة — لا تصفه اللوحة بالطرق');
  assert.equal(shape.observedMeters, 0);
});

test('نجاحٌ واحد على الأقل ⇒ ليس degraded', async () => {
  const { deps } = fakeDeps();
  const shape = await buildRouteShape(denseRun(DAMMAM, 20), deps);
  assert.equal(shape.degraded, false);
});

test('عنقود تذبذبٍ عند عميل (امتداده ~صفر) لا يبتلع نداء مطابقة', async () => {
  const { deps, calls } = fakeDeps();
  // ٨ نقاط داخل ٦٠ متراً = وقوفٌ لا طريق
  const jitter = Array.from({ length: 8 }, (_, i) => at(north(DAMMAM, i * 8), i * 8));
  const shape = await buildRouteShape([...jitter, at(KHOBAR, 3000)], deps);
  assert.ok(MIN_MATCH_EXTENT_M > 60, 'حدّ الامتداد فوق تذبذب الوقوف');
...
test('الميزانية تُنفَق على الأطول امتداداً لا على الأسبق زمنياً', async () => {
  const seen: number[] = [];
  const deps = {
    maxCalls: 1,
    match: async () => null,
  ...
  // وقفة قريبة أولاً (٣ كم) ثم انتقال بعيد (~٣٠ كم): النداء الوحيد للأبعد
...
test('سقف النداءات يُحترم ويُعلَن ولا يُبتلع', async () => {
  ...
  assert.equal(shape.truncated, true, 'ويُعلن أن مقاطع بقيت خاماً');
  assert.ok(shape.segments.length >= 3, 'ومع ذلك يُرسم اليوم كلّه');
});
...
test('جَرْية قصيرة (دون حدّ المطابقة) تصير محطاتِ توجيه لا مقطعاً مرصوداً', async () => {
  const { deps, calls } = fakeDeps();
  // جَرْيتان: كثيفة ثم قصيرة متباعدة (نقطتان)
  const short = [at(KHOBAR, 3000), at(north(KHOBAR, 200), 3060)];
  const shape = await buildRouteShape([...denseRun(DAMMAM, 8, 0), ...short], deps);

  assert.ok(MIN_MATCH_POINTS > short.length, 'الجَرْية القصيرة دون حدّ المطابقة');
