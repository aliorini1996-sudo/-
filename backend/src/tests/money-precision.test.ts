...
test('حارس الصلاحية: رد السعر الشامل لا يتجاوز هامش 0.01 لاي سعر او نسبة', () => {
  // المندوب يرى round(basePrice×(1+t)) ويرسله كما هو؛ الخادم يرده ليقارنه بbasePrice.
  // ان تجاوز الفرق هامش الحارس (0.01) رفض مندوب لم يمس السعر — وهذا ما حدث فعلا.
  const TOL = 0.01;
...
test('الثابت المحاسبي: subtotal − discountAmt + taxAmt = total في الوضعين', () => {
  // العمودان المخزنان يحملان معنى واحدا (صاف قبل الضريبة) مهما كان وضع التسعير،
...
test('البيعة الواحدة تسجل الخصم نفسه من المندوب ومن اللوحة', () => {
  // 10 قطع ب100 صاف @15% وخصم 10%: المندوب يرسل 115 شاملا واللوحة 100 صافيا،
