...
// ————— الدفع الإلكتروني: تمييز مالنا من أمانات الشركات —————
//
// الخطر الحاكم هنا ليس خطأً حسابياً بل خطأ **دلالة**: ما يُحصَّل لعملاء الشركات
// يمرّ بحسابنا ولا نملكه. عدّه إيراداً يضخّم الرقم أضعافاً ويضاعف الضريبة على
// مالٍ ليس لنا — وهو أسهل خطأ يقع في أنظمة «اجمع ثم ورّد».

test('حارس: المحرّك المالي يفصل التحصيل عن العمولة', () => {
  assert.ok(/paylinkCollectedSar/.test(SRC), 'لا فصل للتحصيل نيابةً عن الشركات');
  assert.ok(/kind:\s*'COLLECTED'/.test(SRC), 'لا قراءة لقيود التحصيل');
  assert.ok(/kind:\s*'FEE'/.test(SRC), 'لا قراءة لقيود العمولة');
  // الإيراد = الاشتراكات + العمولة فقط، لا المحصَّل
  assert.ok(/revenueSar\s*=\s*round2\(subsSar\s*\+\s*paylinkFeeSar\)/.test(SRC),
    'الإيراد يضمّ المحصَّل نيابةً عن الشركات — وهو ليس إيرادنا');
});

test('حارس: عمولة البوابة تُحسب على كل ما مرّ بها لا على إيرادنا', () => {
  // ميسر تقتطع نسبتها من الألف المحصَّلة لا من عمولتنا البالغة ٤١ — وحسابها
  // على إيرادنا وحده كان سيُظهر ربحاً وهمياً على خدمة هامشها رقيق أصلاً
  assert.ok(/gatewayBaseSar\s*=\s*round2\(subsSar\s*\+\s*paylinkCollectedSar\)/.test(SRC),
    'أساس عمولة البوابة لا يشمل التحصيل نيابةً عن الشركات');
  assert.ok(/gatewayFee\(gatewayBaseSar\)/.test(SRC), 'العمولة تُحسب على أساس خاطئ');
});

test('حارس: ضريبة العمولة تُقرأ مخزَّنة لا تُعاد استخراجها', () => {
  // feeNet وfeeVat محفوظان على قيد FEE؛ إعادة الاستخراج تُنتج انحرافاً بالهللة
  // بين الإقرار الضريبي ودفتر الأمانات
  assert.ok(/feeVat:\s*true/.test(SRC), 'لا قراءة لضريبة العمولة المخزَّنة');
  assert.ok(/feeNet:\s*true/.test(SRC), 'لا قراءة لصافي العمولة المخزَّن');
});

test('حارس: إجمالي المستحقّ للتوريد يجمع الموجب وحده', () => {
  // جمع السالب مع الموجب كان يُخفي دَيناً على شركة خلف فائض أخرى فيظهر
  // إجمالي التوريد أقلّ من الواجب
  assert.ok(/filter\(\(r\)\s*=>\s*r\.balanceSar\s*>\s*0\)/.test(SRC),
    'إجمالي المستحقّ يجمع الأرصدة السالبة مع الموجبة');
  assert.ok(/negativeCount/.test(SRC), 'الرصيد السالب لا يُعلَن');
});

test('موعد التوريد القادم دائماً خميسٌ قادم لا اليوم', () => {
  // الاثنين ٢٤ أغسطس ٢٠٢٦ → الخميس ٢٧
  const monday = new Date('2026-08-24T09:00:00Z');
  const r = new Date(monday.getTime() + 3 * 3600_000);
  const days = (4 - r.getUTCDay() + 7) % 7 || 7;
  assert.equal(new Date(r.getTime() + days * 86400_000).toISOString().slice(0, 10), '2026-08-27');
  // وإن كان اليوم خميساً فالتالي بعد أسبوع لا اليوم نفسه
  const thu = new Date('2026-08-27T09:00:00Z');
  const rt = new Date(thu.getTime() + 3 * 3600_000);
  const d2 = (4 - rt.getUTCDay() + 7) % 7 || 7;
  assert.equal(d2, 7);
});

test('حارس: أمانات الشركة المطفأة ميزتها لا تختفي من شاشة التوريد', () => {
  // إطفاء paylinkEnabled عن شركة لها رصيد كان يُخفي مالها من شاشة التوريد —
  // وإطفاء ميزة لا يُبرئ ذمّة: الدَّين يبقى حتى يُورَّد ولو توقّفت الخدمة
  assert.ok(/distinct:\s*\['tenantId'\]/.test(SRC), 'لا جمع للشركات ذات القيود');
  assert.ok(/paylinkEnabled:\s*true\s*\}/.test(SRC) && /id:\s*\{\s*in:/.test(SRC),
    'الفلترة بالمفتاح وحده — المطفأة ذات الرصيد تختفي');
});
