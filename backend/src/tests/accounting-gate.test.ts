...
test('الصفحة الرئيسية تخفي كل خانة نقدية عند الإطفاء — لوحةً وجوالاً', () => {
  ...
  // خمسة أقسام نقدية: اليوم · الشهر · المنحنى · أفضل المناديب · أفضل العملاء.
  // («آخر الفواتير» كان سادسها وأُزيل، وحلّت محلّه بلاطات الإدارة — وهي بلا سياج
  //  عن قصد: لا تعرض مبلغاً، والشاشات خلفها محروسة على الخادم بحارسها.)
  assert.equal((mob.match(/\{accountingOn && \(<Section/g) || []).length, 5, 'خمسة أقسام نقدية مسيَّجة');
