// اختبار محرك حساب الفاتورة المشترك (lib/invoiceCalc.ts) — متجهات ذهبية.
//
// ⚠️ هذه المتجهات **عقد** بين الخادم والعميل: النسخة الأمامية web-admin/src/rep/invoiceCalc.ts
// يجب أن تنتج نفس هذه القيم بالضبط. أي تعديل على الصيغة يكسر هذا الاختبار عمدا — عندها
// عدل النسختين معا وحدث القيم هنا. اختلاف كسر واحد = ورقة مطبوعة تخالف سجل الخادم.
...
test('ذهبي: خصم بند 10% — الخصم يخفض الإجمالي لا الضريبة وحدها', () => {
  const r = computeInvoiceTotals(
  ...
  assert.equal(r.discountAmt, 30, 'خصم البند يدخل خصم الفاتورة المعلن');
  // كان 340.5 — أي أعلى من الورقة المطبوعة بمبلغ الخصم كاملا.
  assert.equal(r.total, 310.5, 'الإجمالي = مجموع صافي البنود');
});

test('ذهبي: خصم فاتورة كلي 5% — الضريبة على الوعاء بعد الخصم', () => {
  const r = computeInvoiceTotals(
  ...
  assert.equal(r.taxAmt, 142.5, 'كانت 150 — ضريبة على مبلغ لم يبع به');
  assert.equal(r.total, 1092.5);
});

// ═══ الثوابت التي يجب ألا تنكسر مهما تغيرت الصيغة ═══
// كسر أي منها يعني ورقة مطبوعة تخالف السجل، أو رمز ZATCA بأرقام لا تتسق.

test('ثابت: الإجمالي = مجموع صافي البنود دائما', () => {
