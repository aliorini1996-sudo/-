...
test('المدّة تُحسب من الطابعين لا من رقم العميل', () => {
  // العميل يدّعي 30 ثانية، والطابعان يقولان 300 — نعتمد الخادم
  ...
  assert.equal(r.clientMismatch, true, 'فرق كبير مع رقم العميل يُرفع علماً');
});

test('توافق رقم العميل مع الخادم لا يرفع علماً', () => {
...
/* ─────────────────── السقف: التوكن المنسيّ ─────────────────── */

test('مدّة تفوق ٤ ساعات تُقصّ وتُعلَّم capped', () => {
  const r = computeVisitDuration({ startedAt: base, endedAt: plus(6 * 3600) });
  assert.equal(r.ok, true);
  assert.equal(r.durationSec, MAX_VISIT_SECONDS, 'قُصّت عند السقف');
  assert.equal(r.rawSec, 6 * 3600, 'والخام محفوظ للشفافية');
  assert.equal(r.capped, true);
});

test('عند السقف بالضبط لا تُقصّ', () => {
...
test('مدّة سالبة (endedAt قبل startedAt) تُرفض — لا صفر', () => {
...
test('مدّة أقصر من ثانيتين تُرفض (ضغطة خاطئة)', () => {
  const r = computeVisitDuration({ startedAt: base, endedAt: plus(1) });
  assert.equal(r.ok, false);
  assert.equal(r.durationSec, null);
});

test('طابع زمني فاسد لا يُسقط الدالّة', () => {
  const r = computeVisitDuration({ startedAt: 'ليس تاريخاً', endedAt: plus(300) });
  assert.equal(r.ok, false);
  assert.equal(r.durationSec, null);
  assert.match(String(r.reason), /غير صالح/);
});

test('يقبل ISO نصّاً كما يصل من العميل', () => {
...
test('تنسيق المدّة: دقائق:ثوانٍ وساعات عند اللزوم', () => {
