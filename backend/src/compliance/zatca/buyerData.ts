// ============================================================================
// ZATCA المرحلة الثانية (Z5.1a، D2) — بيانات المشتري في بطاقة العميل: التطبيع، فحص الحفظ، الاكتمال، التصنيف والاقتراح
// ----------------------------------------------------------------------------
// z5_plan §3 Z5.1a + تقرير D2 §9.3–9.5 + نقد الخطة (21) + قرارا المالك Q2/Q3:
//   • التطبيع (كبيانات البائع): قصّ الطرفين، الأرقام العربية-الهندية والفارسية ⇒ لاتينية في الحقول الرقمية، النوع والدولة
//     بأحرف كبيرة، '' ⇒ null. غياب المفتاح = بلا تغيير؛ null = إفراغ.
//   • الفحص عند الحفظ للحقول **المتغيّرة** وحدها (مقارنةً بالمحفوظ بعد تطبيعه): بيانات قديمة خاطئة لا تمنع حفظ غيرها. الصيغ
//     عند وجود القيمة فقط، والنقص لا يمنع الحفظ أبداً (يظهر في قائمة الناقصين).
//   • «مكتمل» = buyerIssues('standard', طرف 01) بلا خطأ — القاعدة نفسها التي يفحص بها الإصدار (لا قاعدة ثانية).
//   • التصنيف (Q2): النوع الصريح أولاً؛ رقم ضريبي أو سجل تجاري ⇒ منشأة تلقائياً؛ معرّف منشأة في حقلَي المعرّف (CRN، 700،
//     MOM…) أو قناة بيع تجارية أو اسم منشأة وحدها ⇒ «غير مصنّف» (الإصدار لا يعدّها منشأة)؛ وإلا فرد. الاقتراح لا يُكتب إلا بفعل
//     صريح من الإدارة.
//   • المندوب يُكمل ولا يُخفّض (نقد الخطة 21): تحويل عميل من ضريبية (01) إلى مبسطة (02) للإدارة وحدها (مع سطر تدقيق في
//     المسار)، وفي نقطة المندوب الضيّقة (Q3) لا يُمسح حقل محفوظ — وبلا صلاحية تعديل العملاء يُكمل الفارغ من حقول Q3 وحدها.
//   • الدولة null = السعودية: SA ⇔ null ليس تغييراً (لا يُعيد فحص بيانات قديمة) ولا مسحاً.
// ⚠️ مكتفٍ بذاته (validators.ts وbuyerParty.ts فقط) ويُنسخ حرفياً (بعد هذا الرأس) إلى web-admin/src/lib/zatca/buyerData.ts.
// ============================================================================
