...
const CITY_BANK: Record<string, string[]> = {
  ...
  'عمان': ['مسقط', 'صلالة', 'صحار', 'نزوى', 'صور', 'البريمي', 'عبري', 'بركاء', 'الرستاق', 'إبراء'],
  'الأردن': ['عمان', 'الزرقاء', 'إربد', 'الرصيفة', 'العقبة', 'السلط', 'مادبا', 'جرش', 'المفرق', 'الكرك'],
...
const KEYWORD_BANK = [
  'تجارة جملة', 'موزع مواد غذائية', 'مستودع توزيع', 'شركة توريدات', 'موزع مشروبات',
  'تاجر جملة', 'موزع مستلزمات طبية', 'موزع منتجات تنظيف', 'موزع مواد بناء', 'موزع قطع غيار',
  'موزع إلكترونيات', 'موزع أدوات منزلية', 'موزع مستحضرات تجميل', 'موزع أغذية معلبة', 'موزع ألبان',
  'شركة لوجستية', 'مركز توزيع', 'موزع قرطاسية', 'موزع أدوات كهربائية', 'موزع أدوية',
...
export async function generateKeywords(country: string, count: number, used: string[]): Promise<string[]> {
  ...
  if (geminiReady()) {
    try {
      const system =
        'أنت خبير تنقيب مبيعات B2B لمنصة Field Sales إدارة مبيعات مناديب التوزيع ' +
        `ولد ${count} مصطلح بحث قصير ومتنوع القطاعات للعثور على شركات جملة/توزيع/توريد في «${country}». ` +
        'استخدم لغة البلد عربية للدول العربية وأضف بعضها بالإنجليزية ' +
        'تجنب تكرار هذه المستخدمة سابقا ' + (used.slice(-40).join(' ') || 'لا شيء') + '. ' +
        'أعد JSON فقط مصفوفة نصوص بلا أي شرح';
      const text = await geminiGenerate(system, `ولد ${count} مصطلحا جديدا ل${country}.`, { maxTokens: 500, temperature: 0.9 });
