...
TOPICS = [
    ...
    {"icon": "invoice", "accent": "green",
     "eyebrow": "أداة مجانية", "title": "فاتورة ضريبية برمز QR خلال 30 ثانية — مجانًا", "benefit": "مولّد مجاني ينشئ فاتورة ضريبية احترافية برمز QR متوافق مع ZATCA وتحمّلها PDF فورًا، بلا تسجيل — على fieldsa.net/invoice-generator.", "pain": "شراء برامج مكلفة لمجرد إصدار فاتورة صحيحة",
     "eyebrow_en": "Free Tool", "title_en": "A ZATCA QR tax invoice in 30 seconds — free", "benefit_en": "A free generator creates a professional tax invoice with a ZATCA-compliant QR code and downloads it as PDF instantly, no signup — at fieldsa.net/invoice-generator.", "pain_en": "buying costly software just to issue a valid invoice"},
]
...
def ask_claude(topic: dict, lang: str) -> str:
    if lang == "en":
        system = (
            ...
            "Write ONE short English tweet (under 270 characters incl. hashtags) in the voice of someone who knows the field — not ad copy.\n"
            "\n"
            "Style (this is what separates professional from generic):\n"
            "• Open with a concrete moment from a rep or manager day, not a marketing question.\n"
            "• One short line resolving it with the feature, then stop. Do not list or explain.\n"
            "• BANNED cliches: the ultimate solution, built for you, revolutionary, game-changer, do not miss out, the future of sales, all-in-one. If a sentence could sell any product, cut it.\n"
            "• NEVER invent a number or percentage (100% compliant, save 40%, 3x faster).\n"
            "• At most one exclamation mark, preferably none. Zero or one emoji.\n"
            "Content rules: real features only from the input below; never claim government certification; ZATCA = Phase 1 only.\n"
            "Mention fieldsa.net when it fits, not every time. Soft CTA.\n"
            "Hashtags: 2-3 ONLY, directly relevant — stuffing reads as automated. Pick from: #FieldSales #VanSales #Distribution #SalesTech #eInvoicing #SaudiArabia.\n"
        ...
        system = (
            ...
            "اكتب تغريدة عربية واحدة قصيرة (أقل من 270 حرفًا شاملة الهاشتاقات) — بصوت إنسان يعرف الميدان، لا بصوت إعلان.\n"
            "\n"
            "الأسلوب (هذا ما يفرّق المنشور المحترف عن الآلي):\n"
            "• ابدأ بمشهد ملموس من يوم المندوب أو المدير، لا بسؤال تسويقي عام. «المندوب واقف عند العميل والشبكة مقطوعة» أفضل من «مللت من الأنظمة القديمة؟».\n"
            "• جملة قصيرة تحلّ المشهد بالميزة، ثم اسكت. لا تشرح ولا تعدّد.\n"
            "• ممنوع منعًا باتًا هذه العبارات المستهلكة: «الحل الأمثل» «مصمم لك» «ثورة» «الأفضل» «لا تفوّت» «انطلق الآن» «مستقبل المبيعات» «نقلة نوعية» «حل متكامل» — أي جملة تصلح لأي منتج آخر احذفها.\n"
            "• ممنوع أي رقم أو نسبة لم تُعطَ لك («توافق 100%» «وفّر 40%» «أسرع بـ3 أضعاف») — اختلاق يُلزم الشركة.\n"
            "• لا تعجّب ولا مبالغة: علامة تعجّب واحدة على الأكثر، ويفضّل صفر. وإيموجي صفر أو واحد.\n"
            "حدود لا تُتجاوز (ادّعاؤها كذب يُلزم الشركة، وهي منشورة على صفحة مزايانا):\n"
            "• مخزون السيارة رقم محسوب (المحمّل ناقص المُباع) لا جرد فعليّ — ممنوع «ما فيه عجز» أو «يضبط نفسه تلقائيًا» أو «يكشف السرقة».\n"
            "• التتبّع يعمل والتطبيق مفتوح ضمن ساعات العمل — ممنوع «تتبّع 24 ساعة» أو «يراقب المندوب طول اليوم».\n"
            "• الفوترة أوف-لاين متاحة في السعودية والخليج، وممنوعة في مصر والإمارات وتونس (الفاتورة تحتاج مقاصّة حكومية لحظية) — لا تعمّم.\n"
            "قواعد المحتوى: ميزات حقيقية فقط من المعطيات أدناه بلا اختلاق؛ لا تدّعِ اعتمادًا من جهة حكومية؛ ZATCA = المرحلة الأولى فقط.\n"
            "الرابط fieldsa.net يُذكر عند المناسبة لا في كل تغريدة، ودعوة الفعل ناعمة.\n"
            "الهاشتاقات: 2–3 فقط وذات صلة مباشرة — الحشو يقلّل المصداقية ويبدو آليًا. اختر من: #المبيعات_الميدانية #التوزيع #مناديب_المبيعات #فان_سيل #الفوترة_الإلكترونية #السعودية #التحول_الرقمي #FieldSales.\n"
            "\n"
            "قبل أن ترسل، تحقّق من ثلاثة أشياء:\n"
            "1) هل نسختَ عناوين المدخل («الميزة» «الفائدة» «ألم العميل»)؟ احذفها — تلك مدخلاتٌ لك لا نصٌّ للنشر. التغريدة نصّ متّصل بلا عناوين ولا نجوم ولا تعداد.\n"
            "2) هل ادّعيتَ «بلا عجز» أو «بدون فروق» أو «يضبط نفسه»؟ احذفه — النظام يُظهر الفرق ويكشف اختلال الحركات، ولا يمنع العجز المادّي.\n"
            "3) هل تجاوزت هاشتاقين أو ثلاثة، أو أكثرتَ الإيموجي؟ اختصر.\n"
...
LOGO_SVG = (
    '<svg width="60" height="60" viewBox="0 0 120 120"><rect width="120" height="120" rx="24" fill="#E15A30"/>'
    '<line x1="32" y1="88" x2="88" y2="32" stroke="#1F1A13" stroke-width="15" stroke-linecap="round"/>'
    '<circle cx="32" cy="88" r="10" fill="#FAF7F0"/><circle cx="60" cy="60" r="8" fill="#FAF7F0"/>'
    '<circle cx="88" cy="32" r="13" fill="#1F1A13"/><circle cx="88" cy="32" r="7" fill="#FAF7F0"/></svg>'
)
