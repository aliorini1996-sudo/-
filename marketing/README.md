...
## كيف يعمل
`اختيار ميزة حقيقية → Claude يكتب التغريدة → تُركّب بطاقة تسويقية بهوية FieldSales وتُصوَّر إلى PNG (16:9، عبر متصفّح headless) → نشر على X`

> الصور **تُرسم بالكود** من هويتك الحقيقية (شعار + ألوان + خطوط IBM Plex) — مطابقة 100% وثابتة، لا ذكاء اصطناعي. عدّل البطاقة في `build_card_html` والمواضيع في `TOPICS` داخل `post.py`.
...
| البطاقة (تُرسم بالكود عبر Playwright) | **مجاني تمامًا** |
