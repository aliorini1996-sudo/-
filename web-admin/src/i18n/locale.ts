...
const PREFIX: Record<Lang, string> = { ar: '', en: '/en', fr: '/fr', tr: '/tr' };
...
export function basePath(pathname: string): string {
  if (pathname === '/en' || pathname === '/fr' || pathname === '/tr') return '/';
  if (pathname.startsWith('/en/') || pathname.startsWith('/fr/') || pathname.startsWith('/tr/')) return pathname.slice(3); // يزيل بادئة اللغة (3 أحرف)
  return pathname;
}
...
// روابط canonical + hreflang البديلة لصفحة ما (بناءً على مسارها العربي) — عربي/إنجليزي/فرنسي/تركي
export function seoUrls(arPath: string, locale: Lang): {
  ...
  const canonical = locale === 'en' ? enUrl : locale === 'fr' ? frUrl : locale === 'tr' ? trUrl : arUrl;
  return {
    canonical,
    alternates: [
