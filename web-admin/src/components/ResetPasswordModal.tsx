...
export default function ResetPasswordModal({ title, subject, onConfirm, onClose }: Props) {
  ...
  const submit = async () => {
    if (pwd.trim().length < 6) { setErr('كلمة المرور 6 أحرف على الأقل'); return; }
    setBusy(true); setErr('');
    try {
      await onConfirm(pwd.trim());
      setDone(pwd.trim());
    } catch (e) {
      setErr((e as { response?: { data?: { message?: string } } })?.response?.data?.message || 'تعذّر إعادة التعيين');
    } finally {
      setBusy(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between p-5 border-b border-[#E9E1D3]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-[#1F1A13]">{done ? 'تم إعادة التعيين' : title}</h2>
        ...
        {done ? (
          ...
          <div className="p-5 space-y-4">
            <p className="text-xs text-[#9C4423] bg-[#FBEBE2] rounded-lg px-3 py-2 leading-relaxed">
              سلّم كلمة المرور الجديدة لصاحب الحساب — لن تظهر مرة أخرى بعد إغلاق النافذة.
            </p>
            ...
            <button onClick={onClose} className="btn-primary w-full justify-center py-2.5">تم</button>
          </div>
        ) : (
          // شاشة الإدخال
          <div className="p-5 space-y-4">
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="label mb-0">كلمة المرور الجديدة</label>
              ...
              <div className="relative">
                <input type={show ? 'text' : 'password'} className="input pl-9" dir="ltr" autoFocus
                  placeholder="6 أحرف على الأقل" value={pwd} onChange={e => { setPwd(e.target.value); setErr(''); }}
