...
test('خصم الفاتورة يُخصم من الوعاء قبل الضريبة', () => {
  // الدلالة المعتمدة (invoiceCalc.ts): «الضريبة = نسبتها من الوعاء بعد كل خصم».
  // النسخة الأولى من هذا الاختبار كانت تتوقّع ضريبةً على الأساس قبل الخصم —
  // أي تحرس الخطأ الذي أصلحته مراجعة الدقّة المالية، وبقيت يتيمةً خارج CI
  // فلم يفضحها أحد. ١٠٠ − ١٠ = ٩٠ وعاء ⇒ ضريبة ١٣٫٥ ⇒ إجمالي ١٠٣٫٥.
  const t = invoiceTotals([line()], 10);
  assert.equal(t.discount, 10);
  assert.equal(t.tax, 13.5);
  assert.equal(t.total, 103.5);
});

test('خصم البند وخصم الفاتورة يجتمعان — والضريبة بعدهما معاً', () => {
  // الوسيط الثاني **نسبة** لا مبلغ (invoiceDiscountPct كما في محرّك الباكند):
  // ١٠٠ − خصم بند ٢٠٪ = ٨٠ ⇒ خصم فاتورة ١٠٪ منها = ٨ ⇒ الخصم الكلّي ٢٨،
  // والوعاء ٧٢ ⇒ ضريبة ١٠٫٨ ⇒ إجمالي ٨٢٫٨
  const t = invoiceTotals([line({ discountPct: 20 })], 10);
  assert.equal(t.discount, 28);
  assert.equal(t.tax, 10.8);
  assert.equal(t.total, 82.8);
});
