...
function RepHome({ user, onQuick, fuelOn, workNumOn, menuOn, accountingOn = true, settingsReady = true, dailyReportOn }: { user: RepUser; onQuick: (s: Screen) => void; fuelOn?: boolean; workNumOn?: boolean; menuOn?: boolean; accountingOn?: boolean; /** وصلت إعدادات الشركة؟ لا نطلب رقماً قبل معرفة المفتاح */ settingsReady?: boolean; dailyReportOn?: boolean }) {
   ...
   * (الرصيد أو الفواتير) كان يمنع ظهور البطاقة كلّها. ويُنادى مع كل تحديث
   * يدويّ أيضاً — بعد فصله كان زرّ «تحديث» لا يحرّك البطاقة، فيسجّل المندوب
   * زيارةً ويضغط تحديث فتبقى «٠ / ٤» حتى يُعيد فتح التطبيق.
   */
...
function CreateReceipt({ customer, repName, company, perms, onClose, onDone }: { customer: any; repName: string; company: Company | null; perms: RepUser; onClose: () => void; onDone: (doc: ReceiptDoc) => void }) {
  ...
  const autoAlloc = (amt?: number, manual = false) => {
    if (manual) setAllocTouched(true);
        ...
        <div>
          ...
          <input type="number" step="0.01" inputMode="decimal" className="input text-lg font-bold" placeholder="0.00"
            ...
            onChange={e => {
              ...
              // إعادة التوزيع تتبع المبلغ ما لم يوزّع المندوب بيده — عندها يبقى
              // ما اتّفق عليه مع العميل ولا تمحوه ضغطةُ مفتاح في حقل المبلغ
              if (!allocTouched) autoAlloc(Number(e.target.value));
            }} />
        </div>
        ...
        {invLoading ? (
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5 gap-2">
              ...
              <button type="button" onClick={() => { setAllocTouched(false); autoAlloc(); }} disabled={!amount || Number(amount) <= 0}
                className="text-xs font-bold text-green-700 disabled:text-gray-300">{tr('توزيع تلقائي على الأقدم')}</button>
            </div>
            <div className="space-y-1.5">
              {openInv.map(inv => (
                <div key={inv.id} className="bg-white border border-gray-200 rounded-xl p-2.5 flex items-center gap-2">
                  ...
                  {/* تاريخ إصدار الفاتورة — عمودٌ مستقلّ: العميل يسأل «سدّدت فاتورة أيّ يوم؟» */}
                  <div className="flex-shrink-0 text-center border-s border-gray-100 ps-2">
                    <p className="text-[10px] text-gray-400 leading-tight">{tr('تاريخ الفاتورة')}</p>
                    <p className="text-[11.5px] font-semibold text-gray-600 leading-tight mt-0.5 whitespace-nowrap">{formatDateShort(inv.invoiceDate)}</p>
                  </div>
            ...
            {Number(amount) > outstanding + 0.004 && (
              <p className="text-[10.5px] text-gray-500 mt-1">
                {tr('الفائض عن مديونية العميل')} ({formatCurrency(r2(Number(amount) - outstanding))}) {tr('يبقى رصيدا دائنا له')}
              </p>
            )}
          </div>
        )}
