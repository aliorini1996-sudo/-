...
function Row({ k, children }: { k: string; children: ReactNode }) {
  return (
    <div className="flex items-center justify-between gap-3 text-[12.5px]">
      ...
      <span className="text-[#1F1A13] text-end">{children}</span>
    </div>
  );
}
...
export function EarningsTab({ me, refreshMe }: TabProps) {
  ...
  const status = (k: 'pending' | 'approved' | 'paid') => <b>{labelOf(COMMISSION_STATUS, k, lang).label}</b>;
    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-[#E9E1D3] bg-white p-3.5 flex gap-2.5 text-[12.5px] text-[#44403a] leading-relaxed">
        ...
        <div className="space-y-1">
          <p>{rich('earn.info1', { pending: status('pending'), approved: status('approved'), days: s.holdDays })}</p>
          <p>{rich('earn.info2', { min: formatSar(s.minPayoutHalalas, lang), paid: status('paid') })}</p>
        </div>
      </div>

      <section>
        <SectionTitle>{t('earn.commissions')}</SectionTitle>
        {commissions.isLoading ? <Loading /> : commissions.isError || !commissions.data ? (
          ...
          <Empty title={t('earn.noCommissions')} hint={t('earn.noCommissionsHint')} />
        ) : (
              ...
              return (
                <div key={c.id} className="bg-white rounded-2xl border border-[#E9E1D3] p-3.5">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <Badge value={labelOf(COMMISSION_STATUS, c.status, lang)} />
                  </div>
                  <div className="mt-2.5 space-y-1.5 border-t border-[#F1EBDF] pt-2.5">
                    <Row k={t('earn.paymentAmount')}><Money h={c.paymentAmountHalalas} /></Row>
                    {(c.refundedHalalas ?? 0) > 0 && <Row k={t('earn.refunded')}><Money h={c.refundedHalalas ?? 0} /></Row>}
                    <Row k={t('earn.rate')}><bdi dir="ltr">{formatRate(c.rateBps)}</bdi></Row>
                    <Row k={t('earn.paymentDate')}>{day(c.paymentPaidAt)}</Row>
                    <Row k={t('earn.holdEnd')}>
                      {left > 0 ? t('earn.holdEndValue', { date: day(c.eligibleAt), days: daysLabel(left, lang) }) : day(c.eligibleAt)}
                    </Row>
                    {c.paidAt && <Row k={t('earn.paidOn')}>{day(c.paidAt)}</Row>}
                  </div>
                  {c.reasonNote && (
                    <p className="mt-2 rounded-xl bg-[#FAF7F0] px-3 py-2 text-[12.5px] text-[#44403a] leading-relaxed" style={{ whiteSpace: 'pre-wrap' }}>
                      <b className="text-[#6E6557]">{t('earn.note')}</b><span dir="auto">{c.reasonNote}</span>
                    </p>
                  )}
                </div>
              );
      ...
      <section>
        <SectionTitle>{t('earn.adjustments')}</SectionTitle>
        {adjustments.isLoading ? <Loading /> : adjustments.isError || !adjustments.data ? (
          ...
          <Empty title={t('earn.noAdjustments')} />
        ) : (
          <div className="space-y-2">
            {adjustments.data.map((a) => (
              <div key={a.id} className="bg-white rounded-2xl border border-[#E9E1D3] px-3.5 py-3">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-[13.5px] font-semibold text-[#1F1A13]">{textOf(ADJUSTMENT_KIND, a.kind, lang)}</span>
                ...
                <div className="flex items-center justify-between gap-2 mt-1 text-[11.5px] text-[#8A8072]">
                  ...
                  <span>{a.settled ? t('earn.settled') : t('earn.nextPayout')}</span>
                </div>
                {a.note && <p className="mt-1.5 text-[12.5px] text-[#44403a]" dir="auto" style={{ whiteSpace: 'pre-wrap' }}>{a.note}</p>}
              </div>
            ))}
          </div>
        )}
            ...
            {payouts.data.map((p) => (
              <div key={p.id} className="bg-white rounded-2xl border border-[#E9E1D3] p-3.5">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <Badge value={labelOf(PAYOUT_STATUS, p.status, lang)} />
                </div>
                <div className="mt-2 space-y-1.5 border-t border-[#F1EBDF] pt-2.5">
                  <Row k={t('earn.commissionsTotal')}><Money h={p.commissionsHalalas} /></Row>
                  {p.adjustmentsHalalas !== 0 && <Row k={t('earn.adjustments')}><Money h={p.adjustmentsHalalas} /></Row>}
                  <Row k={t('earn.transferDate')}>{day(p.transferredAt ?? p.createdAt)}</Row>
                  {p.bankReference && <Row k={t('earn.bankRef')}><bdi dir="ltr" className="break-all">{p.bankReference}</bdi></Row>}
                  <Row k={t('earn.toAccount')}><bdi dir="ltr">{maskedIban(p.ibanLast4)}</bdi></Row>
                </div>
              </div>
            ))}
          </div>
