...
import {
  UtensilsCrossed, Monitor, LayoutGrid, ChefHat, ScrollText, ReceiptText,
  Calculator, Wallet, Boxes, Printer, ArrowLeft, Check, Star, Store, Bike, ShoppingBag,
} from 'lucide-react';

// ============================================================================
// الصفحة التعريفية لعمودية المطاعم «Field Restaurant» (M1)
// مكوّن مستقلّ تماماً — لا يمسّ صفحة هبوط التوزيع المشتركة (LandingPage) إطلاقاً،
// فلا خطر انحدار على fieldsa.net الرئيسية. الهوية عائلية: فحمي/كريمي مشتركان،
// واللون المحوري قرميدي #B5322A + لهجة زعفرانية #E4A11B.
// كل نداءات «ابدأ مجاناً» تذهب إلى /signup?vertical=restaurant (تجربة 10 أيام).
// ============================================================================

const BRICK = '#B5322A';
const BRICK_DARK = '#9A2A23';
const SAFFRON = '#E4A11B';
const VOID = '#1F1A13';
const CREAM = '#FAF7F0';
const GREEN = '#1E7A52';
...
const FEATURES: { icon: React.ElementType; title: string; desc: string }[] = [
  ...
  { icon: ReceiptText, title: 'فوترة إلكترونية معتمدة', desc: 'فاتورة ضريبية مبسّطة برمز QR متوافقة مع هيئة الزكاة والضريبة (ZATCA) السعودية ومنظومة الفواتير المصرية (ETA).' },
...
const ORDER_TYPES: { icon: React.ElementType; label: string }[] = [
  { icon: Store, label: 'طلبات الصالة' },
  { icon: ShoppingBag, label: 'طلبات السفري' },
  { icon: Bike, label: 'طلبات التوصيل' },
];
...
const PLANS: { name: string; price: string; period?: string; limit: string; badge?: string; features: string[]; cta: string }[] = [
  {
    ...
    features: ['كاشير POS وإدارة الطلبات', 'قائمة وطاولات', 'فاتورة إلكترونية ZATCA', 'تقارير أساسية'],
    cta: 'ابدأ الآن',
  },
  {
    name: 'الاحترافية', price: '٧٩٩', period: 'ر.س / شهرياً', limit: 'حتى ٣ نقاط بيع', badge: 'الأكثر طلباً',
    features: ['كل مميزات المبتدئة', 'تذاكر مطبخ KOT/KDS', 'محاسبة وتقارير متقدمة', 'المخزون والوصفات', 'دعم أولوية'],
    cta: 'ابدأ تجربتك المجانية',
  },
  {
    name: 'الفروع', price: 'حسب الطلب', limit: 'فروع ونقاط بيع غير محدودة',
    features: ['كل مميزات الاحترافية', 'إدارة عدّة فروع', 'صلاحيات دقيقة للموظّفين', 'تدريب وإعداد كامل'],
    cta: 'تواصل معنا',
  },
];
...
export default function RestaurantLandingPage() {
  ...
  return (
    <div dir="rtl" style={{ background: CREAM, color: VOID, fontFamily: "'IBM Plex Sans', 'Plex Sans Arabic', sans-serif" }} className="min-h-screen">
      {/* ===== شريط التنقّل ===== */}
      <nav className="sticky top-0 z-40 backdrop-blur border-b" style={{ background: 'rgba(250,247,240,.85)', borderColor: '#E9E1D3' }}>
        <div className="max-w-6xl mx-auto px-5 h-16 flex items-center justify-between gap-4">
          <a href="#top" className="flex items-center gap-2.5">
            <span className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: BRICK }}>
              <UtensilsCrossed size={19} className="text-white" />
            </span>
            <span style={{ fontWeight: 700 }} className="text-lg tracking-tight">
              <span>Field</span><span style={{ color: BRICK }}> Restaurant</span>
            </span>
          </a>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <Link to={SIGNUP} className="text-sm font-bold text-white px-4 py-2 rounded-xl transition-colors" style={{ background: BRICK }}
              onMouseOver={e => (e.currentTarget.style.background = BRICK_DARK)} onMouseOut={e => (e.currentTarget.style.background = BRICK)}>
              ابدأ مجاناً
            </Link>
          </div>
        </div>
      ...
      <header id="top" className="relative overflow-hidden">
        <div className="absolute inset-0" style={{ background: `radial-gradient(120% 120% at 85% 0%, ${BRICK}22, transparent 55%)` }} />
        <div className="relative max-w-6xl mx-auto px-5 pt-16 pb-20 grid lg:grid-cols-2 gap-12 items-center">
          <div>
            <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold mb-5" style={{ background: '#F6E7E5', color: BRICK }}>
              <Star size={13} /> نظام تشغيل مطاعم عربي متكامل
            </span>
            <h1 className="text-4xl sm:text-5xl font-bold leading-[1.15] tracking-tight">
              أدِر مطعمك بالكامل<br /><span style={{ color: BRICK }}>من الكاشير حتى المحاسبة</span>
            </h1>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-3">
              <Link to={SIGNUP} className="inline-flex items-center gap-2 text-white font-bold px-6 py-3.5 rounded-xl transition-colors" style={{ background: BRICK }}
                onMouseOver={e => (e.currentTarget.style.background = BRICK_DARK)} onMouseOut={e => (e.currentTarget.style.background = BRICK)}>
          ...
          <div className="relative">
            <div className="rounded-3xl border shadow-2xl p-5 bg-white" style={{ borderColor: '#E9E1D3' }}>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <span className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: BRICK }}><Monitor size={16} className="text-white" /></span>
              ...
              <div className="flex items-center justify-between mt-1.5"><span className="font-bold">الإجمالي</span><span className="font-bold text-lg tabular-nums" style={{ color: BRICK }}>١١٢٫٧٠ ر.س</span></div>
            ...
            <div className="absolute -bottom-4 -left-4 rounded-2xl px-4 py-3 shadow-xl hidden sm:flex items-center gap-2" style={{ background: VOID }}>
              <ChefHat size={18} style={{ color: SAFFRON }} />
      ...
      <div style={{ background: VOID }}>
        <div className="max-w-6xl mx-auto px-5 py-5 flex flex-wrap items-center justify-center gap-x-10 gap-y-3">
          {ORDER_TYPES.map(({ icon: Icon, label }) => (
            <div key={label} className="flex items-center gap-2 text-[#E8DFD2] text-sm font-semibold">
              <Icon size={18} style={{ color: SAFFRON }} /> {label}
            </div>
          ))}
          <div className="flex items-center gap-2 text-[#E8DFD2] text-sm font-semibold">
            <ReceiptText size={18} style={{ color: SAFFRON }} /> فوترة إلكترونية معتمدة
          </div>
        </div>
      </div>
      ...
      <section id="features" className="max-w-6xl mx-auto px-5 py-20">
        <div className="text-center max-w-2xl mx-auto">
          <div className="text-xs font-semibold tracking-widest uppercase" style={{ color: BRICK }}>المميزات</div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-12">
          {FEATURES.map(({ icon: Icon, title, desc }) => (
            <div key={title} className="rounded-2xl border p-5 bg-white transition-shadow hover:shadow-lg" style={{ borderColor: '#E9E1D3' }}>
              <span className="w-11 h-11 rounded-xl flex items-center justify-center mb-3.5" style={{ background: '#F6E7E5' }}>
                <Icon size={21} style={{ color: BRICK }} />
              </span>
      ...
      {/* ===== كيف يعمل ===== */}
      <section id="how" style={{ background: '#F3ECE0' }}>
        <div className="max-w-6xl mx-auto px-5 py-20">
          <div className="text-center max-w-2xl mx-auto">
            <div className="text-xs font-semibold tracking-widest uppercase" style={{ color: BRICK }}>كيف يعمل</div>
          ...
          <div className="grid md:grid-cols-3 gap-6 mt-12">
            {STEPS.map(({ n, title, desc }) => (
              <div key={n} className="rounded-2xl bg-white border p-6 text-center" style={{ borderColor: '#E9E1D3' }}>
                <span className="w-12 h-12 rounded-2xl inline-flex items-center justify-center text-white text-xl font-bold mb-4" style={{ background: BRICK }}>{n}</span>
      ...
      <section id="pricing" className="max-w-6xl mx-auto px-5 py-20">
        <div className="text-center max-w-2xl mx-auto">
          <div className="text-xs font-semibold tracking-widest uppercase" style={{ color: BRICK }}>الأسعار</div>
        ...
        <div className="grid md:grid-cols-3 gap-5 mt-12 items-stretch">
          {PLANS.map(p => (
            <div key={p.name} className="rounded-2xl border bg-white p-6 flex flex-col relative"
              style={{ borderColor: p.badge ? BRICK : '#E9E1D3', borderWidth: p.badge ? 2 : 1 }}>
              {p.badge && (
                <span className="absolute -top-3 right-6 text-xs font-bold text-white px-3 py-1 rounded-full" style={{ background: BRICK }}>{p.badge}</span>
              )}
              <h3 className="font-bold text-lg">{p.name}</h3>
              <p className="text-sm text-[#9A8F7E] mt-1">{p.limit}</p>
              <div className="mt-4 flex items-end gap-1.5">
                <span className="text-3xl font-bold" style={{ color: BRICK }}>{p.price}</span>
              ...
              <Link to={SIGNUP} className="mt-6 text-center font-bold py-2.5 rounded-xl transition-colors"
                style={p.badge ? { background: BRICK, color: '#fff' } : { border: '1.5px solid #DED5C4', color: VOID }}>
                {p.cta}
              </Link>
            </div>
          ))}
      ...
      {/* ===== الأسئلة الشائعة ===== */}
      <section id="faq" style={{ background: '#F3ECE0' }}>
        <div className="max-w-3xl mx-auto px-5 py-20">
          <div className="text-center">
            <div className="text-xs font-semibold tracking-widest uppercase" style={{ color: BRICK }}>FAQ</div>
            ...
            {FAQ.map(({ q, a }) => (
              <details key={q} className="group rounded-2xl bg-white border p-5" style={{ borderColor: '#E9E1D3' }}>
                <summary className="flex items-center justify-between cursor-pointer font-bold list-none">
                  {q}
                  <span className="text-2xl leading-none transition-transform group-open:rotate-45" style={{ color: BRICK }}>+</span>
                </summary>
      ...
      <section className="relative overflow-hidden" style={{ background: VOID }}>
        <div className="absolute inset-0" style={{ background: `radial-gradient(90% 120% at 80% 0%, ${BRICK}44, transparent 60%)` }} />
        <div className="relative max-w-3xl mx-auto px-5 py-20 text-center">
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
            <Link to={SIGNUP} className="inline-flex items-center gap-2 text-white font-bold px-7 py-3.5 rounded-xl" style={{ background: BRICK }}>
              ابدأ تجربتك المجانية <ArrowLeft size={18} />
            </Link>
        ...
        <div className="max-w-6xl mx-auto px-5 py-12">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-white font-bold text-lg">Field <span style={{ color: BRICK }}>Restaurant</span></span>
            </div>
